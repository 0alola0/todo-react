...
const TaskItem = ({task, toggleCheckmark, deleteTask, enterEdit}) => {
  ...
  return (
    <li className="task drag" id="draggable" draggable="true">
        <input checked={isChecked} type="checkbox" className={isChecked? "completor checked" : "completor"}  onChange={handleCheckox}/>
        <label htmlFor={task.id} className="label-container"><span className={isChecked? "custom-checkbox checked" : "custom-checkbox"} onClick={handleCheckox}>{isChecked&&<CheckIcon  strokeWidth={3.00}/>}</span>{task.name}</label>
